...
import {Layout, Row, Col, Divider, Rate, Carousel, Image, List, Typography, Button, Modal, Input, message, Pagination} from "antd";
...
const News = (parkID) => {
  ...
  const getNewsList = (id) =>{
    axios.get('/api/search_park_news', {params:{parkID: id.parkID}})
      .then((res)=>{
          ...
          <List.Item>
            <Typography>
              <Paragraph>
                ...
                <Row>{moment(item.time*1000).format("YYYY-MM-DD HH:mm")}</Row>
              </Paragraph>
